...
function parseDecisionText(text?: string) { const value = text || "Active strategy cycle"; const action = (value.match(/\b(BUY|SELL|HOLD)\b/i)?.[1] || "HOLD").toUpperCase(); const asset = value.match(/\b(BUY|SELL|HOLD)\s+([A-Z0-9_-]+)/i)?.[2] || "ETH"; const route = value.toLowerCase().includes("uniswap") ? "Uniswap" : "OKX"; return { action, asset, route, rationale: value }; }
...
export default function ArenaPage() {
  ...
  const feed = useMemo<FeedEntry[]>(() => squads.slice(0, 3).map((squad) => { const parsed = parseDecisionText(squad.lastAction); const signal = signalData?.pairs?.find((item) => item.pair === `${parsed.asset}/USDT`) || signalData?.pairs?.[0]; const route: "Uniswap" | "OKX" = signal?.betterRoute === "uniswap" || parsed.route === "Uniswap" ? "Uniswap" : "OKX"; return { id: `${squad.squadId}-${squad.latestTimestamp}`, squadId: squad.squadId, timestamp: squad.latestTimestamp, action: parsed.action, asset: parsed.asset, rationale: parsed.rationale, route, spreadBps: Number(signal?.spreadBps || 0), savedBps: Number(signal?.spreadBps || 0), chain: buildDecisionChain(squad, activityEntries, signal) }; }), [activityEntries, signalData?.pairs, squads]);
