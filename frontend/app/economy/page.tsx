          ...
          {edges.map((edge) => {
            ...
            return (
              <g key={`${edge.id}-label`}>
                ...
                <text x={labelX} y={labelY - 9} textAnchor="middle" fill="#7BC8F6" fontSize="10" fontWeight="700">{edge.paymentType}</text>
                <text x={labelX} y={labelY + 8} textAnchor="middle" fill="#FFFFFF" fontSize="10">{edge.amount}</text>
                <text x={labelX} y={labelY + 22} textAnchor="middle" fill="#9CA3AF" fontSize="8">last: {edge.last}</text>
              </g>
            );
          })}
          ...
          {nodes.map((node) => {
            ...
            return (
              <g key={node.id} transform={`translate(${pos.x - 82} ${pos.y - 42})`}>
                ...
                <text x="82" y="22" textAnchor="middle" fill="#7BC8F6" fontSize="11" fontWeight="700">{node.label}</text>
                <text x="82" y="42" textAnchor="middle" fill="#FFFFFF" fontSize="10">{node.value}</text>
                <text x="82" y="58" textAnchor="middle" fill="#9CA3AF" fontSize="8">
                  {node.secondary.length > 18 ? `${node.secondary.slice(0, 8)}…${node.secondary.slice(-6)}` : node.secondary}
                </text>
              </g>
            );
          })}
