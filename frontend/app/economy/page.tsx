...
function EconomyLoopDiagram({ nodes, edges }: { nodes: EconomyNode[]; edges: EconomyEdge[] }) {
  return (
    <div className="rounded-[32px] border border-black/10 bg-white/70 p-6 dark:border-white/10 dark:bg-white/5">
      <div className="economy-loop-wrapper economy-loop-container overflow-x-auto md:overflow-hidden md:mx-auto md:w-full md:max-w-[980px] md:[transform:scale(0.75)] md:[transform-origin:top_center]">
        <svg viewBox="0 0 640 520" className="min-w-[640px] w-full overflow-visible md:[transform:none]">
          <defs>
          ...
          {edges.map((edge) => {
            ...
            return (
              <g key={`${edge.id}-label`}>
                ...
                <text x={labelX} y={labelY - 8} textAnchor="middle" fill="#7BC8F6" fontSize="9" fontWeight="700">{edge.paymentType}</text>
                <text x={labelX} y={labelY + 8} textAnchor="middle" fill="#FFFFFF" fontSize="9">{edge.amount}</text>
                <text x={labelX} y={labelY + 21} textAnchor="middle" fill="#9CA3AF" fontSize="8">last: {edge.last}</text>
              </g>
            );
          })}
          ...
          {nodes.map((node) => {
            ...
            return (
              <g key={node.id} transform={`translate(${pos.x - 82} ${pos.y - 42})`}>
                ...
                <text x="82" y="22" textAnchor="middle" fill="#7BC8F6" fontSize="10" fontWeight="700">{node.label}</text>
                <text x="82" y="42" textAnchor="middle" fill="#FFFFFF" fontSize="9">{node.value}</text>
                <text x="82" y="58" textAnchor="middle" fill="#9CA3AF" fontSize="8">{node.secondary}</text>
              </g>
            );
          })}
        </svg>
