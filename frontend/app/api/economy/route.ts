...
const STRATEGY_VAULT_ADDRESS = "0x6002767f909B3049d5A65beAD84A843a385a61aC";
const CREATOR_WALLET = "0x795009bb38a32348344a36a4cfcb36e4e84cb8d8";
...
function parseOkbAmount(value: unknown) {
  const numeric = Number(String(value || "0").replace(" OKB", ""));
  return Number.isFinite(numeric) ? numeric : 0;
}

function formatMinutesAgo(timestampSeconds: number) {
...
export async function GET() {
  try {
    const [payments, leaderboard, deployments, x402] = await Promise.all([
      ...
      fetchJson(`${RAW_BASE}/leaderboard.json`).catch(() => ({ squads: [], totalDecisions: 0, updatedAt: null })),
