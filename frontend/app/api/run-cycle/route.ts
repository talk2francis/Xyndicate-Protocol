...
const fetchUniswapPoolPrice = async (okxPrice: number) => {
...
const callOpenAI = async (systemPrompt: string, payload: unknown) => {
...
const routeDecision = (strategistDecision: any, marketData: any) => {
...
const logDecisionOnChain = async (routedDecision: any) => {
...
const recordVaultPnL = async (routedDecision: any) => {
  const privateKey = (process.env.STRATEGIST_KEY || "").trim();
  const rpcUrl = (process.env.XLAYER_RPC || "https://rpc.xlayer.tech").trim();
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const narratorSummary = `Xyndicate Alpha ${
      routedDecision.action === "SELL" ? "trimmed" : routedDecision.action === "HOLD" ? "held" : "deployed"
    } ${routedDecision.asset} (${routedDecision.sizePercent}% treasury) via ${routedDecision.route}. ${routedDecision.rationale}`;
