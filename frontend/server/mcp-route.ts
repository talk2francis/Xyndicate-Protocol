...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tool === "get_leaderboard") return NextResponse.json({ tool, result: await getLeaderboard() });
    if (tool === "get_market_signal") return NextResponse.json({ tool, result: await getMarketSignal(params) });
    if (tool === "get_squad_strategy") return NextResponse.json({ tool, result: await getSquadStrategy(params) });
    if (tool === "execute_route_query") return NextResponse.json({ tool, result: await executeRouteQuery(params) });
